...
class Table(gspread.models.Worksheet):
  ...
  def _parse_header(self):
    ...
    if '' in self.header:
      raise HeaderError('An empty string cannot be used as a field\'s name.')
  ...
  def _record_matches_conditions(self, record, conditions):
    ...
    Returns:
      Bool, specifically True if the record matches all conditions and False
      if not.
    """

    for condition in conditions:
      if len(condition) != 3:
        raise GSpreadDbError('Where conditions must have three elements.')
      field = condition[0]
      operator = _COMPARISON_OPERATOR_MAP[condition[1]]
      compared = str(condition[2])
      if field not in self.header:
        raise GSpreadDbError('Fields in conditions must be in table\'s header.')
  ...
  def delete(self, field=None, value=None, row_numbers=None, where=None):
    ...
    if not any([field, value, row_numbers, where]):
      raise GSpreadDbError('Cannot delete rows without a select mechanism.')
  ...
  def select(self, field=None, value=None, fields=None, as_pandas=True,
    ...
    Conditions can be expressed in two ways: either as a field/value pair or
    as a list of tuple each being an individual condition (where). Using
    field and value is equivalent to be using where = [(field, 'eq', value)].

    The matched records can be returned either as a pd.DataFrame (default), as
    a list of dictionaries or just as a list of row_numbers.

    Args:
      field: str, the field to check.
      value: str, the value to check record[field] against.
      fields: [str], which fields to return. None to get them all.
      as_pandas: bool, whether to return the matched records as pd.DataFrame.
        If False, then records are returned as a list of dictionaries.
    ...
    Returns:
      ...
      as_pandas is True and as_rows False, or list of ints, if as_rows is True.
    """
  ...
  def update(self, field=None, value=None, row_numbers=None,
    ...
    if not any([field, value, row_numbers, where]):
      raise GSpreadDbError('Cannot update records without a select mechanism.')
